...
## Labeling: Manual + Auto

Typing coordinates by hand is impossible — use visual tools:

- **Manual annotation:** [Ultralytics HUB](https://docs.ultralytics.com/hub/), Roboflow, or CVAT — upload images, draw boxes/polygons, export in YOLO format.
- **Auto-labeling with [SAM](https://docs.ultralytics.com/models/sam/)** (Segment Anything): run on raw images to generate masks/boxes, then review & correct. Drastically faster than drawing manually.
...
The fourth failure mode — the one your curves *won't* warn you about.
...
- **Intuition**: A model is `f(image) → number`. Training nudges weights until predictions match reality.
- **Loss**: A smooth measure of "how wrong" — drives gradient descent. YOLO sums three: box, class, distribution.
- **Data Pipeline**: YOLO folder format, `data.yaml`, manual + auto labeling.
- **Kickoff**: Transfer learning + `epochs`, `batch`, `imgsz`, `lr0`, `weight_decay`, `patience`.
- **Diagnose & Fix**: Overfitting, underfitting, class imbalance, data leakage — and the lever for each.
